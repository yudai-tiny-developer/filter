...
function main(app, common, lang) {
    ...
    function updateVisibility(node, deep = true) {
        if (common.isSubscriptions(location.href)) {
            ...
            onNodeLoaded_Shorts(node);
            if (deep) node.querySelectorAll('YTD-RICH-ITEM-RENDERER, YTD-BROWSE').forEach(n => onNodeLoaded_Shorts(n));
        } else if (common.isHistory(location.href)) {
            onNodeLoaded_History(node);
            if (deep) node.querySelectorAll('YT-LOCKUP-VIEW-MODEL, YTM-SHORTS-LOCKUP-VIEW-MODEL-V2, YTD-VIDEO-RENDERER, YTD-BROWSE').forEach(n => onNodeLoaded_History(n));
        } else if (common.isPlaylists(location.href)) {
            onNodeLoaded_Playlists(node);
            if (deep) node.querySelectorAll('YTD-RICH-ITEM-RENDERER, YTD-BROWSE').forEach(n => onNodeLoaded_Playlists(n));
        } else if (common.isPlaylist(location.href)) {
            onNodeLoaded_Playlist(node);
            if (deep) node.querySelectorAll('YT-LOCKUP-VIEW-MODEL, YTD-PLAYLIST-VIDEO-RENDERER, YTD-THUMBNAIL-OVERLAY-RESUME-PLAYBACK-RENDERER, YTD-BROWSE').forEach(n => onNodeLoaded_Playlist(n));
        } else if (common.isHashTag(location.href)) {
            onNodeLoaded_HashTag(node);
            if (deep) node.querySelectorAll('YTD-RICH-ITEM-RENDERER, YTD-BROWSE, TP-YT-APP-HEADER').forEach(n => onNodeLoaded_HashTag(n));
        } else if (common.isChannel(location.href)) {
            onNodeLoaded_Channel(node);
            if (deep) node.querySelectorAll('YTD-CHANNEL-FEATURED-CONTENT-RENDERER, YTD-GRID-VIDEO-RENDERER, YTD-GRID-CHANNEL-RENDERER, YTD-POST-RENDERER, YTM-SHORTS-LOCKUP-VIEW-MODEL-V2, YTD-RICH-ITEM-RENDERER, YT-LOCKUP-VIEW-MODEL, YTD-BACKSTAGE-POST-THREAD-RENDERER, YTD-BROWSE').forEach(n => onNodeLoaded_Channel(n));
        } else if (common.isChannels(location.href)) {
            onNodeLoaded_Channels(node);
            if (deep) node.querySelectorAll('YTD-CHANNEL-RENDERER, YTD-BROWSE').forEach(n => onNodeLoaded_Channels(n));
        } else {
    ...
    function matchQuery(text, node, suggestion_candidate) {
        ...
        return evaluator(normalized.join('\n'));
    }
    ...
    function matchTextContent_Subscriptions_RichItemRenderer(node) {
        ...
        if (title || channel_name) {
            return matchQuery([title?.textContent, channel_name?.textContent], node, true);
        }
    ...
    function matchTextContent_Home_RichItemRenderer(node) {
        ...
        if (post_text || post_author) {
            return matchQuery([post_text?.textContent, post_author?.textContent], node, true);
        }
        ...
        if (ad_metadata) return matchQuery(ad_metadata.textContent, node, false);
    ...
    function matchTextContent_Shorts_RichItemRenderer(node) {
        ...
        const label = metadata?.getAttribute('aria-label');
        if (label) return matchQuery(label, node, true);
    ...
    function matchTextContent_History_ShortsLockupViewModelV2(node) {
        const metadata = node.querySelector('h3.shortsLockupViewModelHostMetadataTitle');
        if (metadata) return matchQuery(metadata.textContent, node, true);
    ...
    function classifyModeStatus_History_VideoRenderer(node) {
        ...
        if (metadata) {
            ...
            if (lang.isLive_metadata(t)) {
                ...
                const shorts =
                    node.querySelector('badge-shape:has(path[d="m17.77 10.32-1.2-.5L18 9.06c1.84-.96 2.53-3.23 1.56-5.06s-3.24-2.53-5.07-1.56L6 6.94c-1.29.68-2.07 2.04-2 3.49.07 1.42.93 2.67 2.22 3.25.03.01 1.2.5 1.2.5L6 14.93c-1.83.97-2.53 3.24-1.56 5.07.97 1.83 3.24 2.53 5.07 1.56l8.5-4.5c1.29-.68 2.06-2.04 1.99-3.49-.07-1.42-.94-2.68-2.23-3.25z"])') ??
                    node.querySelector('badge-shape:has(path[d="m13.974 2.052-8 4.7a4 4 0 00.385 7.097l.942.423-1.327.78a4 4 0 004.053 6.898l8-4.7a4 4 0 00-.385-7.097L16.7 9.73l1.328-.78a4 4 0 00-4.053-6.898Z"])');
                if (shorts) {
                    status.add('short');
                } else {
                    status.add('video');
    ...
    function onNodeLoaded_Playlist(node) {
        switch (node.nodeName) {
            case 'YT-LOCKUP-VIEW-MODEL':
                updateTargetVisibility(node, matchTextContent_Playlist_LockupViewModel, classifyModeStatus_Playlist_LockupViewModel, classifyProgressStatus_Playlist_LockupViewModel);
                break;
    ...
    function matchTextContent_Playlist_LockupViewModel(node) {
        const title = node.querySelector('yt-lockup-metadata-view-model > div:nth-child(2) > h3');
        const channel_name = node.querySelector('yt-content-metadata-view-model > div:nth-child(1) > span:nth-child(1)');
        if (title || channel_name) {
            return matchQuery([title?.textContent, channel_name?.textContent], node, true);
    ...
    function classifyModeStatus_Playlist_LockupViewModel(node) {
        const status = new Set();
    ...
    function classifyProgressStatus_Playlist_LockupViewModel(node) {
        const status = new Set();
    ...
    function matchTextContent_Channel_PostRenderer(node) {
        ...
        if (text) return matchQuery(text.textContent, node, true);
    ...
    function matchTextContent_Channel_BackstagePostThreadRenderer(node) {
        ...
        if (content) return matchQuery(content.textContent, node, true);
    ...
    function matchTextContent_Channels_ChannelRenderer(node) {
        ...
        if (info) return matchQuery(info.textContent, node, true);
        ...
        function createFrequencyMap(inputMap) {
            ...
            for (const [item, node] of inputMap.entries()) {
                ...
                for (let i = 0; i < tokens.length; i++) {
                    ...
                    if (ignoreRegex.test(token)) continue;
    ...
    new MutationObserver((mutations, observer) => {
        for (const m of mutations) {
            updateVisibility(m.target, false);
